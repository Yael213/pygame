...
class Sample:
    ...
    accion_t_minus_3: int
    accion_t_minus_4: int
    accion_t_minus_5: int

    accion: int 
    frames_desde_bala: int


class Juego:
    def limpiar_muestras_minimas(
        self,
        samples: List[Sample],
        min_muestras: int = 10,
        porcentaje_min: float = 0.08
    ) -> List[Sample]:
        """
        Elimina clases con muy pocas muestras.

        Se elimina una acción si:
        - Tiene menos de min_muestras ejemplos
        O
        - Representa menos del porcentaje_min del dataset
        ...
        acciones = [s.accion for s in samples]
        ...
        for accion, cantidad in conteo.items():
            ...
            if (
                cantidad >= min_muestras
                and porcentaje >= porcentaje_min
            ):
        ...
        filtrado = [
            s
            for s in samples
            if s.accion in acciones_validas
        ]
        ...
        for accion, cantidad in conteo.items():
            ...
            print(
                f"Acción {accion}: "
                f"{cantidad} muestras "
                f"({cantidad/total:.1%}) -> {estado}"
            )

        print(
            f"Dataset limpio: "
            f"{len(filtrado)} / {total}"
        )
    ...
    def __init__(self) -> None:
        ...
        self.ultima_accion_guardada = None
    ...
    def _reset_estado_juego(self) -> None:
        ...
        self.historial_acciones = [
            ...
            ACCION_NADA,
            ACCION_NADA
        ]# Reset de memoria a corto plazo
    ...
    def registrar_decision_manual(self) -> None:
        ...
        if not hasattr(self, "_frames_desde_registro"):
            self._frames_desde_registro = 0

        self._frames_desde_registro += 1

        if (
            accion_actual == self.ultima_accion_guardada
            and self._frames_desde_registro < 5
        ):
            return

        self._frames_desde_registro = 0

        self.ultima_accion_guardada = accion_actual
        ...
        self.datos_modelo.append(
            Sample(
                ...
                accion_t_minus_3=self.historial_acciones[2],
                accion_t_minus_4=self.historial_acciones[3],
                accion_t_minus_5=self.historial_acciones[4],

                accion=accion_actual,
                frames_desde_bala=self._frames_desde_registro
            )
        )

        if accion_actual != self.historial_acciones[0]:
            self.historial_acciones = [
                accion_actual,
                self.historial_acciones[0],
                self.historial_acciones[1],
                self.historial_acciones[2],
                self.historial_acciones[3]
            ]

    
    def entrenar_modelo(self) -> Tuple[bool, str]:
        samples = self.limpiar_muestras_minimas(
            list(self.datos_modelo),
            min_muestras=1,
            porcentaje_min=0.10
        )
        ...
        if len(samples) < 100:
            return False, (
                "Muy pocos datos después de limpiar "
                "las clases pequeñas."
            )

        if len(set(s.accion for s in samples)) < 2:
            return False, (
                "Después de limpiar quedó una sola acción."
            )

        if len(samples) < 100:
            return False, "Necesitas más datos (< 100)."

        # Mantener memoria temporal
        X = [
            [
                s.velocidad_bala,
                s.distancia,
                ...
                float(s.accion_t_minus_1),
                float(s.accion_t_minus_2),
                float(s.accion_t_minus_3),
                float(s.accion_t_minus_4),
                float(s.accion_t_minus_5),
            ]
            for s in samples
        ]
        ...
        return (
            True,
            f"MLP Secuencial entrenado | accuracy≈{train_acc:.3f}"
        )
    ...
    def decision_auto(self) -> int:
        ...
        # Predicción secuencial usando MLP + memoria temporal en los frames anteriores
        X = [[
            float(self.velocidad_bala),
            float(distancia),
            float(self.tipo_bala_actual),

            float(self.historial_acciones[0]),
            float(self.historial_acciones[1]),
            float(self.historial_acciones[2]),
            float(self.historial_acciones[3]),
            float(self.historial_acciones[4]),
        ]]
        ...
        if accion != self.historial_acciones[0]:
            self.historial_acciones = [
                accion,
                self.historial_acciones[0],
                self.historial_acciones[1],
    ...
    def _dibujar_menu(self, msg: str = "") -> None:
        ...
        titulo = self.fuente.render("MENÚ - MLP SECUENCIAL", True, self.BLANCO)
        ...
        opciones = [
            ...
            "T - Entrenar MLP (Aprenderá secuencias)",
